...
namespace Final
{
    public class Program
    {
        ...
        public const int ChromosomesEachGene = 10000;
            ...
            Chromosome initializeChoromosome()
            {
                ...
                foreach (Branch originalBranch in Branches)
                {
                    ...
                    foreach (Shift originalShift in originalBranch.Shifts)
                    {
                        ...
                        shiftsCopy.Add(shiftCopy); // Add the copy to the list!
                    }
        ...
        public static double calaulateChoromosomeFitness(Chromosome ch)
        {
            ...
            double calculatesShiftFitness(Shift shift)
            {
                ...
                fitness += shift.AssignedEmployees.Sum(empId =>
                {
                    ...
                    return emp == null ? 0 :
                        (emp.backUprequestedShifts.Contains(shift.Id) ? 3 : -5);
                });
            ...
            // Calculate fitness directly from shifts in the chromosome
            foreach (var entry in ch.Shifts)
            {
                List<Shift> shifts = entry.Value;
